...
Given a model $p_{\bm{\theta}}$ with parameters $\bm{\theta}$ and a dataset $\mathcal{D}$ the log-likelihood objective of a general flow-based model is equivalent to minimizing:
$$
\mathcal{L}(\mathcal{D}) = \frac{1}{N}\sum^N_{i=1}-\log p_{\bm{\theta}}(\textbf{x}^{(i)})
$$
Regarding flow-based generative models, the generative process \cite{dinh2014nice, dinh2016density} is defined as:
$$
\textbf{z} \sim p_{\bm{\theta}}(\textbf{x}), \quad \textbf{x} = \textbf{g}_{\bm{\theta}}(\textbf{z})
$$
where $\textbf{z}$ a sample in the latent space and $p_{\bm{\theta}}(\textbf{z})$ is its density, which for simplicity is usually defined as a spherical multivariate Gaussian distribution $p_{\bm{\theta}}(\textbf{z}) = \mathcal{N}(\textbf{z}; 0, \textbf{I})$. The decoding function $\textbf{g}_{\bm{\theta}}(\cdot)$ is invertible, i.e. one can infer the mapping in the latent space from an input variable as follows: $\textbf{z} = \textbf{f}_{\bm{\theta}}(\textbf{x}) =  \textbf{g}^{-1}_{\bm{\theta}}(\textbf{x})$. In flow-based models, the invertible function $\textbf{f}_{\bm{\theta}}$ (or $\textbf{g}_{\bm{\theta}}$) is composed by a sequence of invertible transformations: $\textbf{f}_{\bm{\theta}} = \textbf{f}_{1} \circ \textbf{f}_{2} \circ ... \circ \textbf{f}_K$. Such sequence of transformations is named \textit{normalizing flow} \cite{rezende2015variational}.

Using this assumptions on the encoder-decoder function $\textbf{f}_{\bm{\theta}}$, one can rewrite the probability density of a flow-based model given a datapoint using _the change of variables rule_.
$$
\log p_{\bm{\theta}}(\textbf{x}) = \log p_{\bm{\theta}}(\textbf{z}) + \sum_{i=1}^K \log |\det(d\textbf{h}_i/d\textbf{h}_{i-1})|
$$
